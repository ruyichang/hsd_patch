...
class RPC extends RPCBase {
  ...
  async _handleWork(data) {
    ...
    this.logger.debug('_handleWork mask          : %x', mask);
    this.logger.debug('_handleWork hdr.maskHash(): %x', hdr.maskHash());


    if (!mask) {
      this.logger.debug('_handleWork mask error        : %x', mask);
      return [false, 'stale'];
    }

    const {nonce, time, extraNonce} = hdr;
    this.logger.debug('_handleWork nonce       : %d', nonce);
    this.logger.debug('_handleWork extraNonce  : %x', extraNonce);
    this.logger.debug('_handleWork hdr.time    : %d', hdr.time);


    const proof = attempt.getProof(nonce, time, extraNonce, mask);
    const block = attempt.commit(proof);        //----------------------
    this.logger.debug('>>>block hash    : %x', block.hash());
    this.logger.debug('>>>attempt.target: %x', attempt.target.toString('hex'));
    this.logger.debug('>>>this.network  : %x', this.network.toString('hex'));
    //|
    if (!proof.verify(attempt.target, this.network)) {                 //|
      this.logger.debug('---------bad-diffbits-----------');         //|
  ...
  async _createWork() {
    ...
    this.logger.debug('++++++++++++++++_createWork maskHash      : %x', maskHash);
    this.logger.debug('++++++++++++++++_createWork mask          : %x', mask);
    this.logger.debug('++++++++++++++++_createWork data.mask     : %x', data.mask);
    this.logger.debug('++++++++++++++++_createWork data._maskHash: %x', data._maskHash);
    this.logger.debug('++++++++++++++++_createWork fees: %d',attempt.fees);
