...
class RPC extends RPCBase {
  ...
  async _handleWork(data) {
    ...
    this.logger.info('-------------------------_handleWork--------------------------------');
    this.logger.info('_handleWork data     : %s', data);
    ...
    const mask = this.maskMap.get(hdr.mask);
    ...
    this.logger.info('_handleWork time        : %d', time);
    this.logger.info('_handleWork mask        : %x', mask);
    ...
    if (!proof.verify(attempt.target, this.network)){
      this.logger.info(' proof.verify attempt.target: %x', attempt.target);
      this.logger.info(' proof.verify this.network  : %x', this.network);
      this.logger.info(' proof.verify bad-diffbits!');
      this.logger.info(' ---1 block hash  : %x', block.hash());
      return [false, 'bad-diffbits'];
    }
    this.logger.info(' ---2 block hash  : %x', block.hash());

    // const block = attempt.commit(proof);
    this.logger.info(' ---3 block hash  : %x', block.hash());
    ...
    this.logger.info('Add block hash  : %x', block.hash());
