...
class RPC extends RPCBase {
  ...
  async _handleWork(data) {
    ...
    this.logger.info('RPC _handleWork-data from miner : %s.', data);
    ...
    this.logger.info('_handleWork hdr.mask: %x', hdr.mask);
    this.logger.info('_handleWork hdr.namehash: %x', hdr.namehash);
    ...
    if (!hdr.prevBlock.equals(attempt.prevBlock)
      ...
      this.logger.info('RPC hdr.bits !== attempt.bits,hdr.bits: %d , attempt.bits: %d.', hdr.bits, attempt.bits);
      return [false, '1-stale'];
    }
    this.logger.info('RPC hdr.bits == attempt.bits,hdr.bits: %d , attempt.bits: %d.', hdr.bits, attempt.bits);

    // const mask = this.maskMap.get(hdr.maskHash());
    // this.logger.info('this.maskMap.get(hdr.maskHash(): %x', mask);
    const mask = hdr._maskHash;

    if (!mask){
      this.logger.info('RPC hdr.maskHash(): %x.', hdr.maskHash());
      this.logger.info('RPC mask: %x.', mask);
      return [false, '2-stale'];
    }

    const {nonce, time, extraNonce} = hdr;
    // for debug logs
    this.logger.info('RPC nonce: %d', nonce);
    this.logger.info('RPC time: %d', time);
    this.logger.info('RPC extraNonce: %x', extraNonce);
    this.logger.info('RPC mask: %x', mask);
    const proof = attempt.getProof(nonce, time, extraNonce, mask);

    this.logger.info('RPC proof: %x', proof.hash());


    if (!proof.verify(attempt.target, this.network)){
      this.logger.info('bad-diffbits:RPC attempt.target: %x', attempt.target);
      this.logger.info('bad-diffbits:RPC this.network: %x', this.network);
      return [false, 'bad-diffbits'];
    }

    this.logger.info('RPC attempt.target: %x', attempt.target);
    this.logger.info('RPC this.network: %x', this.network);
