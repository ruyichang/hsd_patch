...
class RPC extends RPCBase {
  ...
  async _handleWork(data) {
    ...
    this.logger.info('---------------------------------------------------------');
    this.logger.info('_handleWork hdr.version     : %d', hdr.version);
    this.logger.info('_handleWork hdr.prevBlock   : %x', hdr.prevBlock);
    this.logger.info('_handleWork hdr.merkleRoot  : %x', hdr.merkleRoot);
    this.logger.info('_handleWork hdr.witnessRoot : %x', hdr.witnessRoot);
    this.logger.info('_handleWork hdr.treeRoot    : %x', hdr.treeRoot);
    this.logger.info('_handleWork hdr.reservedRoot: %x', hdr.reservedRoot);
    this.logger.info('_handleWork hdr.time        : %d', hdr.time);
    this.logger.info('_handleWork hdr.bits        : %d', hdr.bits);
    this.logger.info('_handleWork hdr.nonce       : %d', hdr.nonce);
    this.logger.info('_handleWork hdr.extraNonce  : %x', hdr.extraNonce);
    this.logger.info('_handleWork hdr.mask        : %x', hdr.mask);       // 00000XXXX...//I set it to all 0
    this.logger.info('_handleWork hdr._maskHash   : %x', hdr._maskHash);  // black2b with previous hash
    this.logger.info('_handleWork hdr._hash       : %x', hdr._hash);
    this.logger.info('---------------------------------------------------------');
    ...
    this.logger.info('_handleWork mask          : %x', mask);
    this.logger.info('_handleWork hdr.maskHash(): %x', hdr.maskHash());


      if (!mask){
        this.logger.info('_handleWork mask error        : %x', mask);
        return [false, 'stale'];
    }

    const {nonce, time, extraNonce} = hdr;
    this.logger.info('_handleWork nonce       : %d', nonce);
    this.logger.info('_handleWork extraNonce  : %x', extraNonce);
    this.logger.info('_handleWork hdr.time    : %d', hdr.time);
    ...
    this.logger.info('>>>block hash  : %x', block.hash());            //|
                                                                      //|
    if (!proof.verify(attempt.target, this.network)){                 //|
        this.logger.info('---------bad-diffbits-----------');         //|
  ...
  async _createWork() {
    const [mask, attempt, maskHash] = await this.updateWork();
    const time = attempt.time;
    const data = attempt.getHeader(0, time, consensus.ZERO_NONCE, mask);

      this.logger.info('++++++++++++++++_createWork maskHash      : %x', maskHash);
      this.logger.info('++++++++++++++++_createWork mask          : %x', mask);
      this.logger.info('++++++++++++++++_createWork data.mask     : %x', data.mask);
      this.logger.info('++++++++++++++++_createWork data._maskHash: %x', data._maskHash);
      this.logger.info('++++++++++++++++_createWork fees: %d', data.attempt.getHeader.fee);
    ...
    return {
      ...
      fee: data.attempt.getHeader.fee
    };
  }
  ...
  async updateWork() {
    ...
    if (attempt) {
      ...
      this.logger.info ('updateWork [attempt]     mask: %x.', mask);
      this.logger.info ('updateWork [attempt] maskHash: %x.', maskHash);
      return [mask, attempt];
    }
    ...
    this.logger.info ('updateWork     mask: %x.', mask);
    this.logger.info ('updateWork maskHash: %x.', maskHash);
    return [mask, attempt, maskHash];
  }
