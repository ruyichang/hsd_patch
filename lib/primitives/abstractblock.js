...
class AbstractBlock extends bio.Struct {
  ...
  constructor() {
    ...
    this._hash = null;
    this._maskHash = null;
    // this.logger = node.logger.context('AbstractBlock');
  }
  ...
  maskHash() {
    if (this._maskHash != null){
      // this.logger.info('maskHash--this._maskHash: %x', this._maskHash);
      console.log('----------------maskHash--this._maskHash: %x', this._maskHash);
      return this._maskHash;
    }

    // Hash with previous block in case a pool wants
    // to re-use the same mask for the next block!
    // this.logger.info('BLAKE2b.multi(this.prevBlock, this.mask): %x', BLAKE2b.multi(this.prevBlock, this.mask));
    console.log('---------------BLAKE2b.multi(this.prevBlock, this.mask): %x', BLAKE2b.multi(this.prevBlock, this.mask));
  ...
  fromMiner(data) {
    ...
    // this.logger.info('fromMiner hdr.padding: %x',this.padding(20));
    // this.logger.info('fromMiner padding    : %x',padding);
